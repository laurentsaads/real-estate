...
function getLBCData(lbcUrl, routeResponse, callback) {
    request(lbcUrl, function (error, response, html) {               //fonction anonyme
        if (!error) {
            ...
            if (lbcData) {
                ...
                routeResponse.render('home', { error: 'No data found' });
            }
        }
        else {
            routeResponse.render('home', { error: 'Error loading the given URL' });
        }
    });
}
